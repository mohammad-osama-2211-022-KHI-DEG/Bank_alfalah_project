...
while True:
    ...
    if (len(Angry_face_ids_current_frame) != previous_unique_Angry_count) or (len(faces) != previous_footfall_number):
        response = send_emotion_data(Angry_count, len(Angry_face_ids_current_frame), len(faces))
    ...
    if len(Angry_face_ids_current_frame) >= 1 :
        response = send_notification(f"number of unique Angry count is {len(Angry_face_ids_current_frame)}", "Agressive_customers")
        print(response.status_code, response.content)
