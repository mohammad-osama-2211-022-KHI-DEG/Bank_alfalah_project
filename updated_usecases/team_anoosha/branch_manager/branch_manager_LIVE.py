...
REAPPEARANCE_THRESHOLD = 10.0
ID_DISAPPEAR_THRESHOLD = 1.0
ENCODINGS_FILE = '/home/zubair/Xloop/bafl/face_recognition/branch_manager/encodings/face_enc_proper_name.pkl'
LOG_FILENAME = f'/home/zubair/Xloop/bafl/face_recognition/branch_manager/video/output/{FILENAME}.log'
VIDEO_PATH = 0
...
def send_data_on_threshold(name):
    ...
    try:
        data = {
            ...
            "exitTimestamp": datetime.now(timezone(timedelta(hours=5))).strftime('%Y-%m-%dT%H:%M:%S') + str('+05:00')
        }
        ...
        if response is not None:
            if response and response.status_code // 100 == 2:
                ...
                logging.info(f"Request failed with status code {response.raise_for_status()}")
        else:
            pass
...
def check_and_generate_alerts():
    ...
    for name, last_appearance_time in last_appearance_times.items():
        ...
        if elapsed_time_since_last_detected > ID_DISAPPEAR_THRESHOLD:
            ...
            logging.warning(f"\033[91mAlert for Detected Person: {name}, Total Duration: {round(total_duration, 2)} seconds\033[0m")

def process_face(face, rgb,  frame, font):
    """
...
def main():
    ...
    try:
        ...
        while True:
            ...
            for face in faces:
                process_face(face, rgb,  frame, font)
            check_and_generate_alerts()
            out.write(frame)
            cv2.imshow(f"{FILENAME}_{FACE_RECOGNITION_TOLERANCE}_tolerance_{REAPPEARANCE_THRESHOLD}sec", frame)
