...
PROCESS_FRAMES_PER_SECOND = 24
NUM_JITTERS = 10
CLASS_LABELS = ["Not Happy", "Happy", "Not Happy", "Not Happy", "Not Happy"]
LOG_FILE_NAME = f"/home/zubair/Xloop/bafl/ready_github_code/team_anoosha/happy_count/output/logs/{FILENAME}.log"
VIDEO_FILE_PATH = f"/home/zubair/Xloop/bafl/ready_github_code/team_anoosha/happy_count/video/{FILENAME}.mp4"
OUTPUT_VIDEO_PATH = f"/home/zubair/Xloop/bafl/ready_github_code/team_anoosha/happy_count/output/{FILENAME}_dist_{DISTANCE_THRESHOLD}.mkv"
EMOTION_DETECTION_MODEL_PATH = "/home/zubair/Xloop/bafl/ready_github_code/team_anoosha/happy_count/Emotion_Detection.h5"
...
def send_emotion_data(happy_count, unique_smile_count, footfall_number):
    ...
    try:
        ...
        logging.info(f"Happy Data Request_payload:{data}")
        response = requests.post(url, json=data, headers=HEADERS)
        return response
    except requests.RequestException as e:
        logging.error(f"An error occurred while sending the request: {e}")
...
while True:
    ...
    if current_frame_number % process_interval == 0:
        ...
        for face in faces:
            ...
            if (len(happy_face_ids_current_frame) != previous_unique_happy_count) or (
                ...
                previous_unique_happy_count = len(happy_face_ids_current_frame)
                previous_footfall_number = len(foot_fall_ids_current_frame)

            # Send notifications
            if (
                len(happy_face_ids_current_frame) >= 1
                and len(happy_face_ids_current_frame) % 1 == 0
                and len(happy_face_ids_current_frame) // 1 > notification_counter
            ):
                ...
                if response is not None:
                    ...
                    logging.error("Failed to get a response")
    # Write information about Happy Counts, Unique Happy Counts and Foot_fall Number
    cv2.putText(
        frame,
    ...
    cv2.putText(
        frame,
        f"Unique Happy Count: {len(happy_face_ids_current_frame)}",
        (20, 80),
        cv2.FONT_HERSHEY_SIMPLEX,
        1,
        (0, 0, 0),
