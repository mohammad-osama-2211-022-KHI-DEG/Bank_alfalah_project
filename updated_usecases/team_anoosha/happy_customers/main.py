...
while True:
    ...
    for face in faces:
        ...
        if np.sum([roi_gray]) != 0:
            ...
            label_position = (x, y - 10)  # Adjust the position to be just above the face
            cv2.putText(frame, f'{recognized_person_id} - {label}', label_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 2)
    ...
    if (len(happy_face_ids_current_frame) != previous_unique_happy_count) or (len(faces) != previous_footfall_number):
        response = send_emotion_data(happy_count, len(happy_face_ids_current_frame), len(faces))
    ...
    # Send notifications
    if len(happy_face_ids_current_frame) >= 1 and len(happy_face_ids_current_frame) % 1 == 0 and len(
            happy_face_ids_current_frame) // 1 > notification_counter:
        response = send_notification(f"number of unique happy count is {len(happy_face_ids_current_frame)}", "happy_customers")
        print(response.status_code, response.content)
    ...
    cv2.putText(frame, f"Happy Count: {happy_count}", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2,
                cv2.LINE_AA)
    cv2.putText(frame, f"Unique Happy Count: {len(happy_face_ids_current_frame)}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1,
