...
def check_status_and_avglevel(detected_tables_info):
    ...
    if detected_tables_info:
        ...
        # Check if there is at least one table detected
        if detected_tables_info:
            last_added_item = list(detected_tables_info.values())[-1]
            avglevelMess_value = last_added_item['avglevelMess']
...
def check_for_tables(results_tables):
    # Check if any tables are detected
    if len(results_tables.boxes) == 0:
        print("No tables detected in the current frame.")
        return False
    return True


def process_frame(frame, model_objects, model_tables, threshold,
                  detected_tables_info, previous_status, tables_and_windows):
    H, W, _ = frame.shape
    output_width = 640
    output_height =480
    ...
    if not check_for_tables(results_tables):
    ...
    cv2.imshow("Branch Thread Main", resized_frame_objects)
    
    return detected_tables_info, previous_status, tables_and_windows



def process_frame_branch(frame):
    model_path_objects = os.path.join('Branch\\objects_weights\\best.pt')
    model_path_tables = os.path.join('Branch\\table_weights\\best.pt')

    model_objects = YOLO(model_path_objects)
    model_tables = YOLO(model_path_tables)
    threshold = 0.5

    # cv2.namedWindow("YOLO Detections Branch", cv2.WINDOW_NORMAL)
    # cv2.resizeWindow("YOLO Detections Branch", 800, 600)
    ...
    detected_tables_info, previous_status, tables_and_windows = process_frame(
            frame, model_objects, model_tables, threshold,
            detected_tables_info, previous_status, tables_and_windows
        )
