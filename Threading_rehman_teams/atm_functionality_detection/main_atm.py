...
def detect_atm_usage(model_path, video_path, target_fps):
    ...
    while cap.isOpened():
        ...
        if not success:
            logger.info(f"Working with frame {frame_no}, no frame detected")
            break
        else:
            process_frame(frame, model, atm_detected, total_working_count, total_notworking_count, counter, url, headers)
...
def process_frame(frame, model, atm_detected, total_working_count, total_notworking_count, counter, url, headers):
    ...
    for result in results:
        ...
        complainBoxAvailable = alert_complainbox_telephone["complaintBoxAvailable"]
        ...
        if len(res) != 0 and not atm_detected:
            ...
            if 1.0 in class_name and res[1.0] >= 0.8:
                ...
                data = data_preparation(workingStatus, total_working_count, total_notworking_count, complainBoxAvailable, telephoneAvailable, timestamp)
                response = requests.post(url, json=data, headers=headers)
                logger.info("Endpoint response status code: %s", response.status_code)
            elif count >= 1000:
                logger.info("ATM detection timeout reached. Exiting...")
                counter += 1
...
def complainbox_telephone(class_name):
    logger.info('Checking for complain-box and telephone...')
    # Complain-box & Telephone are present
    if 2.0 in class_name and 3.0 in class_name:
        logger.info('Both complain-box and telephone are present.')
    ...
    # Complain-box is present & Telephone is not present
        ...
        logger.info('Complain-box is present but telephone is not present.')
        alert= {"complaintBoxAvailable": True,
        "telephoneAvailable": False}
        return alert
    # Complain-box is not present & Telephone is present
    elif 2.0 not in class_name and 3.0 in class_name:
        logger.info('Complain-box is not present but telephone is present.')
    ...
    # Complain-box & Telephone are not present
        ...
        logger.info('Neither complain-box nor telephone are present.')
        alert= {"complaintBoxAvailable": False,
        "telephoneAvailable": False}
        return alert      
def Time():
...
def Alert(timestamp):
    ...
    logger.info("Endpoint response status code:", response.status_code)
