...
def build_payload(timestamp, clean_status):
    return {
        "country": "pakistan",
        "branch": "clifton",
        "city": "karachi",
...
def process_frame(frame, model_objects, model_tables, output_width, output_height, threshold,
    ...
    for i, result in enumerate(results_tables.boxes.data.tolist()):
        ...
        if score > threshold:
            ...
            detected_tables_info[i] = {'id': i + 1, 'coordinates': (x1_orig, y1_orig, x2_orig, y2_orig), 'status': "CLEAN",'timestamp': timestamp}
    ...
    if len(set(status_history)) == 2:
        ...
        payload = build_payload(timestamp, cleanliness_status)
...
def main():
    video_path = os.path.join('SMVD.mp4')
