    ...
    if success:
        ...
        for track_id in track_ids:
            if track_id not in entry_times:
                ...
                data_to_push = {
                        'track_id': 'PK-ISL-JA-'+ str(track_id),
                        #'track_id': track_id,
                        'time': formatted_time,
                        'status': 'present',
                        'country': 'pakistan',
        ...
        # for track_id in list(entry_times.keys()):
        #     duration = current_time - entry_times[track_id]
        #     if duration >= 300 and track_id not in logs_track_list:
        #         formatted_duration = round(duration, 2)
        #         logs_dict = {
        #                 'track_id': 'PK-ISL-JA-'+ str(track_id),
        #                 'usecase': 'customer_waiting_time',
        #                 #'track_id': track_id,
        #                 'time': formatted_time,
        ...
        cv2.imshow("Customer Wait Time Tracking, Clifton, Karachi", annotated_frame)
